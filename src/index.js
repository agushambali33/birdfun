...
async function submitScoreToBlockchain(score) {
  ...
  try {
    ...
    // non-blocking: keep UI interactive while waiting
    tx.wait().then(async () => {
      try { playerPoints = await contract.playerPoints(playerAddress); } catch { playerPoints = ethers.BigNumber.from(0); }
      try { rewardPreview = await contract.getRewardPreview(playerAddress); } catch { rewardPreview = ethers.BigNumber.from(0); }
      toggleWeb3UI();
      ...
      console.warn('submitPoints wait failed', e);
      // keep UI alive, show error if needed
...
async function redeemPoints() {
  ...
  try {
    ...
    tx.wait().then(async () => {
      ...
      console.warn('redeem wait failed', e);
...
function createTopBarUI() {
  ...
  // claim button moved into topbar to be inline with others
  ...
  topBar.appendChild(claimToggle);
